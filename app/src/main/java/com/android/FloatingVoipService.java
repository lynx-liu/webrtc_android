...
import com.webrtc.session.CallSession.CallSessionCallback;
import com.webrtc.session.EnumType.CallEndReason;
import com.webrtc.session.EnumType.CallState;
import com.android.core.util.Utils;
import com.android.webrtc.BuildConfig;
...
public class FloatingVoipService extends Service {
    ...
    public void onCreate() {
        ...
        screenSize = Utils.getScreenSize(getApplicationContext());
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        isStarted = true;
        session = App.getInstance().getAvEngineKit().getCurrentSession();
        ...
        public boolean onTouch(View v, MotionEvent event) {
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE: {
                    ...
                    params.x = screenSize.x - (int) (event.getRawX() - startX) - view.getWidth();
                    //这里修复了刚开始移动的时候，悬浮窗的y坐标是不正确的，要减去状态栏的高度
                    params.y = (int) (event.getRawY() - startY - statusBarHeight);
                    if (params.x < margin) params.x = margin;
                    if (params.x > screenSize.x - margin) params.x = screenSize.x - margin;

                    if (params.y + view.getHeight() + statusBarHeight > screenSize.y - margin)
                        params.y = screenSize.y - statusBarHeight - view.getHeight();
    ...
    private void refreshCallDurationInfo(TextView timeView) {
        CallSession session = App.getInstance().getAvEngineKit().getCurrentSession();
